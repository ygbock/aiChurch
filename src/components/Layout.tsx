...
import { setDoc, doc, collection, query, where, onSnapshot, updateDoc, orderBy, limit } from 'firebase/firestore';
...
export default function Layout() {
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      ...
      if (target.closest('.menu-trigger') || target.closest('.menu-content')) return;
                  ...
                  {showNotifications && (
                    ...
                    >
                      <div className="p-4 border-b border-slate-100 flex items-center justify-between shrink-0 bg-slate-50/50">
                        <h3 className="text-sm font-black text-slate-800 uppercase tracking-tight">System Notifications</h3>
                      ...
                      <div className="flex-1 overflow-y-auto no-scrollbar py-2">
                        {notifications.length > 0 ? (
                          notifications.map((notif) => (
                            ...
                            >
                              <div className="flex gap-3">
                                <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform shadow-sm ${
                                  notif.category === 'event' ? 'bg-amber-100 text-amber-600' :
                                  notif.category === 'task' ? 'bg-emerald-100 text-emerald-600' :
                                  notif.category === 'chat' ? 'bg-indigo-100 text-indigo-600' :
                                  notif.category === 'alert' ? 'bg-rose-100 text-rose-600' :
                                  'bg-blue-100 text-blue-600'
                                }`}>
                                  {notif.category === 'event' ? <CalendarDays size={18} /> :
                                   notif.category === 'task' ? <CheckCircle2 size={18} /> :
                                   notif.category === 'chat' ? <MessageSquare size={18} /> :
                                   notif.category === 'alert' ? <Shield size={18} /> :
                                   <Clock size={18} />}
                                </div>
                                <div className="min-w-0 flex-1">
                                  <div className="flex justify-between items-start mb-0.5">
                                    <p className="text-xs font-black text-slate-900 uppercase tracking-tight truncate">{notif.title}</p>
                                    <span className="text-[8px] font-black text-slate-300 uppercase tracking-widest">
                                      {notif.createdAt ? format(notif.createdAt.toDate ? notif.createdAt.toDate() : new Date(notif.createdAt), 'HH:mm') : 'NOW'}
                                    </span>
                                  </div>
                                  <p className="text-[11px] text-slate-500 leading-tight line-clamp-2">{notif.message || notif.description || 'Notification summary'}</p>
                                  <div className="flex items-center justify-between mt-2">
                                    <div className="flex items-center gap-1.5">
                                      <div className={`w-1 h-1 rounded-full ${
                                        notif.category === 'event' ? 'bg-amber-400' :
                                        notif.category === 'task' ? 'bg-emerald-400' :
                                        'bg-blue-400'
                                      }`} />
                                    ...
                                    <span className="text-[8px] font-black text-blue-500 uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">Interact</span>
                                  </div>
                                </div>
                              </div>
                            </div>
                          ...
                          <div className="py-16 px-6 text-center space-y-3">
                            ...
                            <div>
                              <p className="text-sm font-black text-slate-900 uppercase tracking-tight">Void State</p>
                              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">No active notifications detected</p>
                            </div>
                          </div>
                        )}
                      </div>
