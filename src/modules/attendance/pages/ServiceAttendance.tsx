  ...
  const toggleAttendance = async (member: Member) => {
    ...
    try {
      if (attendance[member.id]) {
        ...
        systemEvents.publish(Events.ATTENDANCE_CHECKED_IN, {
            memberId: member.id,
            fullName: member.fullName,
            programName: activeEvent.title,
          ...
          <div>
            ...
            {activeEvent && (
              <p className="text-slate-400 font-medium mt-2 flex items-center gap-2">
                <Calendar size={16} /> {activeEvent.title} — {safeFormat(activeEvent.date || Date.now(), 'EEEE, MMM d')}
              </p>
            )}
          </div>
              ...
              <div className="space-y-2">
                 {recentEvents.map(event => (
                   ...
                   >
                     ...
                     <p className="text-[10px] font-medium text-slate-500">{safeFormat(event.date || Date.now(), 'MMM d, yyyy')}</p>
                   </button>
                 ))}
              </div>
           </div>
