...
export default function Calendar() {
  ...
  useEffect(() => {
    ...
    const generateSlots = () => {
      ...
      const dayName = format(safeParseISO(bookingDate), 'EEEE');
  ...
  useEffect(() => {
    ...
    const checkReminders = () => {
      ...
      const upcoming = appointments.filter(app => {
        try {
          const appDate = safeParseISO(`${app.date}T${app.time}`);
  ...
  const combinedEvents = React.useMemo(() => {
    ...
    events.forEach(evt => {
      ...
      const untilLimit = evt.recurrence.until ? safeParseISO(evt.recurrence.until) : addYears(start, 1);
    ...
    unsubscribe = onSnapshot(q, (snapshot) => {
      const docs = snapshot.docs.map(doc => {
        ...
        if (eventDate?.toDate) eventDate = eventDate.toDate();
          ...
          const parsed = safeParseISO(eventDate);
          eventDate = isNaN(parsed.getTime()) ? new Date() : parsed;
        } else if (!(eventDate instanceof Date) || isNaN(eventDate.getTime())) {
          eventDate = new Date();
        }
                ...
                >
                  ...
                  <div className="space-y-2.5 pt-4 border-t border-slate-100 mb-6">
                    <div className="flex items-center gap-3 text-slate-600">
                      ...
                      <span className="text-[13px] font-medium">{appt.date ? format(safeParseISO(appt.date), 'EEE, MMM d, yyyy') : ''}</span>
                    </div>
                    <div className="flex items-center gap-3 text-slate-600">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      <div className="flex items-center gap-1 text-[10px] text-slate-400">
                        ...
                        <span>{rem.date ? format(safeParseISO(rem.date), 'MMM d') : ''}</span>
                      </div>
                      <div className="flex items-center gap-1 text-[10px] text-slate-400">
                        <Clock size={12} />
                        <span>{rem.time}</span>
                ...
                <div className="text-slate-500">
                  {selectedSlot && (
                    <p className="text-xs font-bold flex items-center gap-2">
                      ...
                      <span>{format(safeParseISO(bookingDate), 'MMM d')} @ {selectedSlot}</span>
                    </p>
                  )}
                </div>
                <button
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-col gap-3">
                      <div className="flex items-center gap-3 text-slate-600">
                        ...
                        <span className="text-sm font-bold">{selectedAppointment?.date ? format(safeParseISO(selectedAppointment.date), 'PPPP') : ''}</span>
                      </div>
                      <div className="flex items-center gap-3 text-slate-600">
