...
const safeFormat = (dateObj: any, formatStr: string) => {
  if (!dateObj) return '';
  try {
    const d = new Date(dateObj.seconds ? dateObj.toMillis() : dateObj);
...
import { TagEditorModal } from '../registry/components/TagEditorModal';
import { MemberGivingTab } from '../registry/components/MemberGivingTab';
import { MemberAttendanceTab } from '../registry/components/MemberAttendanceTab';
import { MemberBioTab } from '../registry/components/MemberBioTab';
import { MemberSignalTab } from '../registry/components/MemberSignalTab';
import { MemberDiscipleshipTab } from '../registry/components/MemberDiscipleshipTab';
import { PastoralCareTab } from '../registry/components/PastoralCareTab';
    ...
    const fetchMember = async () => {
      ...
      try {
        ...
        if (!foundMember) {
          ...
          let validDocs = snapMemberId.docs.filter(d => d.ref.path.includes('districts/'));
          if (validDocs.length === 0) validDocs = snapUid.docs.filter(d => d.ref.path.includes('districts/'));
          if (validDocs.length === 0) validDocs = snapId.docs.filter(d => d.ref.path.includes('districts/'));
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 sm:gap-y-4 gap-x-12 mt-6">
              ...
              <div className="flex items-center gap-3 text-slate-600 justify-center md:justify-start">
                ...
                <span className="text-[11px] sm:text-sm font-medium tracking-wide">Joined {member.createdAt ? (
                  ...
                  typeof member.createdAt === 'string' ? safeFormat(member.createdAt, 'MMM d, yyyy') :
                  'Jan 1, 2026'
                ) : 'Jan 1, 2026'}</span>
              </div>
            </div>
                 ...
                 {activities.map((act, i) => (
                    <div className="flex gap-4" key={act.id}>
                       ...
                       <div className="pb-6 w-full">
                          <p className="text-sm text-slate-500 font-bold mb-1">
                             {act.timestamp ? format(act.timestamp?.toDate ? act.timestamp.toDate() : new Date(act.timestamp.seconds ? act.timestamp.seconds * 1000 : act.timestamp), 'MMM d, yyyy - h:mm a') : 'Now'}
                          </p>
