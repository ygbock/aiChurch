...
export function useMembers() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(mQuery, (snapshot) => {
      // Filter out 'members' subcollections that might exist in other modules (like ministryChannels)
      const validDocs = snapshot.docs.filter(d => d.ref.path.startsWith('districts/'));
      const mList = validDocs.map(d => {
