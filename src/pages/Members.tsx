    ...
    >
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 flex flex-col lg:flex-row gap-4 items-center">
        ...
        <div className="flex gap-2 w-full lg:w-auto">
          <button className="flex-1 lg:flex-none flex items-center justify-center gap-2 px-4 py-2.5 bg-white border border-slate-200 rounded-lg text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors">
            <Filter size={18} />
            Filters
          </button>
          ...
          {isBranchAdmin ? (
            <select 
              className="flex-1 lg:flex-none bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm font-medium text-slate-600 outline-none focus:ring-1 focus:ring-blue-600 appearance-none min-w-[140px]"
            >
            ...
            <select 
              className="flex-1 lg:flex-none bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm font-medium text-slate-600 outline-none focus:ring-1 focus:ring-blue-600 appearance-none"
            >
              {role === 'district' ? (
                <>
                  <option>All District Branches</option>
                  <option>Central Cathedral</option>
                  <option>Eastside Ministry</option>
                  <option>Grace Chapel</option>
                </>
              ) : (
                <>
          ...
          {isSuperAdmin && (
            ...
            >
              ...
              <option>North America District</option>
              <option>West Africa District</option>
              <option>Europe District</option>
            </select>
          )}
        </div>
      </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredMembers.map((member: MemberData) => (
                  <MemberTableRow 
                    ...
                    branch={isBranchAdmin ? (member.level || 'Convert') : 'Main Campus'} 
...
const MemberTableRow: React.FC<MemberTableRowProps> = ({ id, name, email, phone, branch, status, statusType }) => {
  const statusClasses = {
