...
const mockActivities = [
  {
    id: 1,
    type: "member",
    text: "New member registered in Westside branch",
    time: "2h ago",
  },
  {
    id: 2,
    type: "leader",
    text: "Admin access granted for South Valley",
    time: "5h ago",
  },
  {
    id: 3,
    type: "branch",
    text: "Quarterly report submitted by Grace Chapel",
    time: "1d ago",
  },
  {
    id: 4,
    type: "transfer",
    text: "Transfer request approved for John Doe",
    time: "2d ago",
  },
];
...
export default function DistrictDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "branches" | "leadership" | "members" | "baptism" | "security"
  >("overview");
  ...
  useEffect(() => {
    ...
    const unsubscribeDistrict = onSnapshot(
      districtRef,
      (doc) => {
        if (doc.exists()) {
          const data = doc.data();
    ...
    const unsubscribeMembers = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.forEach((doc) => {
          const data = doc.data();
          const isVisitor =
            data.level === "Visitor" || data.membershipLevel === "visitor";
          const isConvert =
            data.level === "Convert" || data.membershipLevel === "convert";
          if (!isVisitor && !isConvert) {
            results.push({ id: doc.id, ...data } as MemberData);
          }
        });
        setMembers(results);
      },
    ...
    const unsubscribeBaptism = onSnapshot(
      baptismQuery,
      (snapshot) => {
        const results: any[] = [];
        snapshot.forEach((doc) => {
  ...
  const handleCreateBranch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingBranchId) {
        await updateDoc(
          doc(db, "districts", districtId, "branches", editingBranchId),
          branchData,
        );
      } else {
      ...
      handleFirestoreError(
        error,
        editingBranchId ? OperationType.UPDATE : OperationType.CREATE,
        "branches",
      );
    } finally {
      setSaving(false);
  ...
  const handleDeleteBranch = async (branchId: string) => {
    if (
      !districtId ||
      !window.confirm(
        "Are you sure you want to delete this branch? This is permanent.",
      )
    )
      return;
    try {
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-blue-100 text-sm">
                  Your administrative account is ready, but your record in the
                  member registry is incomplete.
                </p>
              </div>
            </div>
            <button
        ...
        <div className="w-full lg:w-auto">
          ...
          <div className="flex items-center justify-between lg:justify-start gap-3">
            <h2 className="text-2xl font-bold text-slate-900">
              {districtData?.name || "District Oversight"}
            </h2>
            <button
          ...
          <p className="text-slate-500 text-sm mt-1">
            Managing {branches.length} branches across the{" "}
              ...
              <div className="flex justify-between items-center mb-8">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Cumulative membership growth across all branches
                  </p>
                </div>
                    ...
                    <Bar dataKey="members" radius={[4, 4, 0, 0]}>
                      {mockPerformanceData.map((entry, index) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={
                            index === mockPerformanceData.length - 1
                              ? "#2563eb"
                              : "#94a3b8"
                          }
                        />
              ...
              <div className="bg-slate-900 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden group">
                <div className="relative z-10">
                  ...
                  <p className="text-slate-400 text-sm mb-4">
                    You are at 85% of your quarterly expansion target.
                  </p>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
            <div className="px-6 py-5 border-b border-slate-200 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4">
              <div className="w-full xl:w-auto">
                ...
                <p className="text-sm text-slate-500">
                  Live summary of all satellite locations
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 md:table w-full text-left border-collapse gap-3 md:gap-0 p-3 md:p-0">
                <div className="hidden md:table-header-group">
                  <div className="md:table-row bg-slate-50/50">
                    ...
                    <div className="md:table-cell px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-widest text-right">
                      Actions
                    </div>
                  </div>
                </div>
                      ...
                      <div className="lg:table-cell px-6 py-12 text-center text-slate-400">
                        <div className="flex flex-col items-center gap-2">
                          ...
                          <p className="text-sm font-medium">
                            No branches established in this district yet.
                          </p>
                          <button
          ...
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
            <div className="px-6 py-5 border-b border-slate-200 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4">
              <div className="w-full xl:w-auto">
                ...
                <p className="text-sm text-slate-500">
                  Complete directory of members across all branches
                </p>
              </div>
              <div className="flex flex-col sm:flex-row items-center gap-3 w-full xl:w-auto">
                <div className="relative w-full sm:flex-1 xl:w-64">
              ...
              <div className="px-6 py-3 bg-blue-50 border-b border-blue-100 flex items-center justify-between animate-in slide-in-from-top duration-300">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="px-3 py-1 bg-white border border-blue-200 rounded-lg text-[10px] font-bold text-blue-600 hover:bg-blue-600 hover:text-white transition-all"
                  >
                    Assign Role
                  </button>
                  <button
                  ...
                  <button
                    ...
                    className="p-1 px-2 text-blue-400 hover:text-blue-600 text-[10px] uppercase font-bold"
                  >
                    Cancel
                  </button>
                </div>
                  ...
                  {members.length === 0 ? (
                    ...
                    members
                      ...
                      .map((member) => (
                        <div
                          key={member.id}
                          className="bg-white border border-slate-100 rounded-2xl md:rounded-none md:border-none p-3 md:p-0 hover:bg-slate-50 transition-all flex flex-col md:table-row min-w-0"
                        >
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              {member.photoUrl ? (
                                <img
                                  src={member.photoUrl}
                                  alt=""
                                  className="w-8 h-8 md:w-10 md:h-10 rounded-full md:rounded-xl object-cover border-2 border-slate-100 shrink-0"
                              ...
                              <div className="flex flex-col min-w-0">
                                ...
                                <div className="flex items-center gap-2 mt-0.5">
                                  ...
                                  <span className="text-[9px] text-slate-300 md:hidden">
                                    •
                                  </span>
                          ...
                          <div className="relative md:table-cell px-6 py-4 text-right">
                            <MemberActionDropdown
                              ...
                              onEdit={() =>
                                navigate(
                                  `/members/edit/${member.id}?districtId=${member.districtId || profile?.districtId}&branchId=${member.branchId}`,
                                )
                              }
                              onView={() =>
                                navigate(
                                  `/members/profile/${member.id}?districtId=${member.districtId || profile?.districtId}&branchId=${member.branchId}`,
                                )
                              }
                            />
                          </div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm">
            <div className="px-6 py-5 border-b border-slate-200">
              <h3 className="text-lg font-bold text-slate-900">
                Baptism Approval Workflow
              </h3>
              <p className="text-sm text-slate-500">
                Review candidates submitted by branches for baptism approval.
              </p>
            </div>
              ...
              {baptismRequests.length === 0 ? (
                ...
                <div className="space-y-4">
                  {baptismRequests.map((req) => (
                    <div
                      key={req.id}
                      className="group bg-slate-50 rounded-2xl border border-slate-100 p-4 hover:border-blue-200 transition-all"
                    >
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              <span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full uppercase truncate max-w-[100px]">
                                {req.branch || "Unknown Branch"}
                              </span>
                              <span className="text-[10px] font-bold text-slate-500 bg-slate-200/50 px-2 py-0.5 rounded-full uppercase">
                                {req.baptismStatus}
                              </span>
                            </div>
                          </div>
                        ...
                        <div className="flex items-center gap-2 w-full md:w-auto">
                          {req.baptismStatus === "Submitted to District" ? (
                            ...
                            <span className="text-xs font-bold text-slate-400 bg-slate-100 px-4 py-2 rounded-xl">
                              Wait for HQ Final Approval
                            </span>
                          )}
                          <button
                            onClick={() =>
                              navigate(
                                `/members/edit/${req.memberId}?districtId=${req.districtId}&branchId=${req.branchId}`,
                              )
                            }
            ...
            <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
              <div className="px-6 py-6 border-b border-slate-200 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Authorized administrators and branch managers
                  </p>
                </div>
                <button
      ...
      <div className="px-0 md:px-6 py-2 md:py-4 block md:table-cell min-w-0">
        <div className="flex items-center gap-3 md:gap-3">
          ...
          <div className="flex flex-col min-w-0 flex-1">
            <span className="text-sm md:text-sm font-bold text-slate-800 group-hover:text-blue-600 transition-all truncate">
              {name}
            </span>
            <span className="text-[10px] text-slate-400 font-medium uppercase tracking-wider truncate block md:hidden">
              {location}
            </span>
          </div>
        </div>
      </div>
      <div className="hidden md:table-cell px-6 py-4 text-sm text-slate-600">
        {location}
      </div>
      <div className="hidden md:table-cell px-6 py-4 text-sm font-bold text-emerald-600">
        {capacity}
      </div>
